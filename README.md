# Kan-I
This idea for this project came about when I was trying to demonstrate a privilege escalation vulnerability in an EKS cluster. When trying to run `kubectl auth can-i --list` it would unhelpfully always return no permissions with the message: `Warning: the list may be incomplete: webhook authorizer does not support user rule resolution.`. Yet when running specific `kubectl auth can-i <verb> <resource>` commandsit would correctly repsond yes/no, so I threw this together to get a full list of permissions when `kubectl` won't do it for me. 
